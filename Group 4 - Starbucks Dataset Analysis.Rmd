...
#there has been updates to the transform function since initially
#creating this project
#hence, hard-coded corrections are made below:
train_nn_data3 = train_nn[,3][1]
train_nn_data4 = train_nn[,4][1]
train_nn_data6 = train_nn[,6][1]
train_nn_data8 = train_nn[,8][1]
train_nn_data = cbind(train_nn_data3, train_nn_data4, 
                      train_nn_data6, train_nn_data8)
test_nn = nn_data[-divide_rows,]
test_nn_data3 = test_nn[,3][1]
test_nn_data4 = test_nn[,4][1]
test_nn_data6 = test_nn[,6][1]
test_nn_data8 = test_nn[,8][1]
test_nn_data = cbind(test_nn_data3, test_nn_data4, 
                      test_nn_data6, test_nn_data8)

#checking total_fat_g,  calories and sugar_g
predictions = knn(train = train_nn_data[,c(1,2,4)], test = test_nn_data[,c(1,2,4)],
                  cl = train_nn[,2], k = 7)
#add in total_carbs_g as per the logistic model
predictions2 = class::knn(train = train_nn_data, test = test_nn_data, 
...
for(i in 1:1000){
  ...
  predictions = knn(train = train_nn_data[,c(1,2,4)], 
                    test = test_nn_data[,c(1,2,4)],
                    cl = train_nn[,2], k = 7)
  predictions2 = knn(train = train_nn_data, test = test_nn_data,
                    cl = train_nn[,2], k = 7)
...
nn_metrics = c(mean(nn_accuracies), mean(nn_recalls), 
               mean(nn_precisions))
nn_metrics2 = c(mean(nn_accuracies2), mean(nn_recalls2), 
                mean(nn_precisions2))
